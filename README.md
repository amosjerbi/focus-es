...
Selected interface assets used by Focus were sourced or adapted from
[P-10 Menu for ES-DE](https://github.com/anthonycaccese/p-10-menu-es-de),
created by Anthony Caccese. Credit and ownership of those assets remain with
their original project and contributors.

For the intended layout, select `focus-es` as the Theme Set and leave
`Gamelist View Style` on `Automatic`; Focus declares `GameCarousel` as its
default view. Then enable `Quick System Select` and `Start On Gamelist`.
In the horizontal carousel, D-pad Left/Right changes
games and L2/R2 changes platforms. `Start On System` chooses which platform's
games appear at startup. The layout is optimized for 640x480.
